...
module.exports.run = (client, message, args) => {
    if (!message.guild.member(message.author).hasPermission('KICK_MEMBERS')) { return message.channel.send('T\'as cru tu pouvais m\'utiliser comme ça ?!'); }
    ...
    if (message.mentions.users.size === 0) { return message.channel.send('Vous devez mentionner un utilisateur !'); }
        ...
        message.mentions.users.first().send(`ça dégage !`)
            .then(() => {
                kickMember.kick()
                    .then((member) => {
                        message.channel.send(`Dites bye-bye à ${member.user.username} , il à été kick par ${message.author.username} !`);
                    })
                        .catch((err) => {
